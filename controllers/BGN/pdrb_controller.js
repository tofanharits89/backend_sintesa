import pdrbmodel from '../../models/mbg/pdrbmodel.js';
import db from '../../config/Database8.js'; // atau db dari sequelize
import { decryptData } from '../../middleware/Decrypt.js';
import Sequelize from "sequelize";

export const getFilteredPdrbEncrypted = async (req, res) => {
  // Ambil dan dekripsi parameter queryParams
  const queryParams = req.query.queryParams;
  console.log("[PDRB] Incoming queryParams:", queryParams);
  if (!queryParams) {
  ...
  try {
    ...
    console.log("[PDRB] Decrypted query:", decryptedQuery);
    // decryptedQuery diharapkan dalam bentuk query SQL
  } catch (error) {
    console.error("[PDRB] Failed to decrypt queryParams:", error);
  ...
  try {
    ...
    console.log("[PDRB] Executing resultsQuery:", resultsQuery);
    console.log("[PDRB] Executing totalCountQuery:", totalCountQuery);
    ...
    console.error("[PDRB] Error executing query:", error);
    res.status(500).json({
