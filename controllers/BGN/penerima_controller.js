import penerimaModel from '../../models/mbg/penerimamodel.js';
import db from '../../config/Database8.js';
import { decryptData } from '../../middleware/Decrypt.js';
import Sequelize from "sequelize";

// Endpoint untuk query terenkripsi + pagination (by_penerima_detail)
export const getFilteredPenerimaEncrypted = async (req, res) => {
  const queryParams = req.query.queryParams;
  console.log("[PENERIMA] Incoming queryParams:", queryParams);
  if (!queryParams) {
  ...
  try {
    ...
    console.log("[PENERIMA] Decrypted query:", decryptedQuery);
  } catch (error) {
    console.error("[PENERIMA] Failed to decrypt queryParams:", error);
  ...
  try {
    ...
    console.log("[PENERIMA] Executing resultsQuery:", resultsQuery);
    console.log("[PENERIMA] Executing totalCountQuery:", totalCountQuery);
    ...
    console.error("[PENERIMA] Error executing query:", error);
    res.status(500).json({
