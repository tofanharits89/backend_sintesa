import spasialModel from '../../models/mbg/spasialmodel.js';
import db from '../../config/Database8.js';
import { decryptData } from '../../middleware/Decrypt.js';
import Sequelize from "sequelize";

// Endpoint untuk query terenkripsi + pagination (data_spasial_mbg)
export const getFilteredSpasialEncrypted = async (req, res) => {
  const queryParams = req.query.queryParams;
  console.log("[SPASIAL] Incoming queryParams:", queryParams);
  if (!queryParams) {
  ...
  try {
    ...
    console.log("[SPASIAL] Decrypted query:", decryptedQuery);
  } catch (error) {
    console.error("[SPASIAL] Failed to decrypt queryParams:", error);
  ...
  try {
    ...
    console.log("[SPASIAL] Executing resultsQuery:", resultsQuery);
    console.log("[SPASIAL] Executing totalCountQuery:", totalCountQuery);
    ...
    console.error("[SPASIAL] Error executing query:", error);
    res.status(500).json({
