import Inflasi_Tembakau_Model from "../../models/mbg/inflasi_tembakau_model.js";
import { decryptData } from "../../middleware/Decrypt.js";
import db from "../../config/Database8.js";
import { Sequelize } from "sequelize";

// GET - Mengambil semua data inflasi tembakau
export const getInflasiTembakau = async (req, res) => {
    console.log("Request to get all inflasi tembakau data received", req.query);
  try {
    ...
    res.json({
      success: true,
      message: "Data inflasi tembakau berhasil diambil",
      result: results,
      total: results.length
    });

  } catch (error) {
    console.error("Error getting inflasi tembakau data:", error);
    
    // Response error
    res.status(500).json({
      success: false,
      message: "Terjadi kesalahan saat mengambil data inflasi tembakau",
      error: error.message
    });
  } finally {
    console.log("Finished processing request to get inflasi tembakau data");
  }
};
