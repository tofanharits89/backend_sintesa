import summarymodel from '../../models/mbg/summary.js';
...
export const getFilteredSummaryEncrypted = async (req, res) => {
  const queryParams = req.query.queryParams;
  console.log("[SUMMARY] Incoming queryParams:", queryParams);
  if (!queryParams) {
  ...
  try {
    ...
    console.log("[SUMMARY] Decrypted query:", decryptedQuery);
  } catch (error) {
    console.error("[SUMMARY] Failed to decrypt queryParams:", error);
  ...
  try {
    ...
    console.log("[SUMMARY] Executing resultsQuery:", resultsQuery);
    console.log("[SUMMARY] Executing totalCountQuery:", totalCountQuery);
    ...
    console.error("[SUMMARY] Error executing query:", error);
    res.status(500).json({
